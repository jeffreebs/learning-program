...
def save_in_csv(video_games ):
    with open ("video_games.csv", "w") as file:
        ...
        writer = csv.DictWriter(file, fieldnames = space, delimiter="\t")
...
def read_and_print ():
    with open("video_games.csv", "r", newline="", ) as file:
        reader = csv.DictReader(file, delimiter="\t")
        for row in reader:
            print(f"{row['name']}\t{row['gender']}\t{row['developer']}\t{row['classification']}")
